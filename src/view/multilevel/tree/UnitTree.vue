<template>
  <div class="checkTreeBox">
    <Tree @on-select-change="selectUnit" :render="isEdit?renderContent:null" :data="unitTree"
          :load-data="getChildUnits"></Tree>

    <Modal v-model="isAddUnit" title="机构新增" footer-hide draggable>
      <p style="margin-left:10%">机构名称:&nbsp;&nbsp;&nbsp;<Input v-model="unitInfo.name" placeholder="机构名称"
                                                               style="width: 50%"/></p>
      <br>
      <p v-show="isAdd" style="margin-left:5%">机构开始时间:&nbsp;&nbsp;<DatePicker v-model="unitInfo.beginTime" type="date"
                                                                              placeholder="机构开始时间"
                                                                              style="width: 48%"></DatePicker>
      </p>
      <br>
...
export default {
  ...
  methods: {
    initUnitTree () {
      this.$http
        ...
        .then(response => {
          ...
          console.log(ret + '触发了treeStaffList')
          this.$emit('tree-list', ret)
          if (ret === 1) {
            this.unitTree = '暂无数据!!!'
            return
          }
    ...
    getChildUnits (code, callback) {
      this.$http
        .get('/api/pc/tree/getUnitTree?unitId=' + code.id)
        .then(response => {
          var ret = response.body.ret
          if (ret === 1) {
            item.expand = false
    ...
    addUnit () {
      ...
      if (this.unitInfo.name === undefined || this.unitInfo.name.trim() === '') {
      ...
      this.$http
        ...
        .then(function (response) {
          ...
          if (response.body.ret === 0) {
            that.$http
              .get('/api/pc/tree/getUnitTree?unitId=' + that.addData.id)
              .then(response => {
                var ret = response.body.ret
                if (ret === 0) {
                  const children = that.getTree(response.body.data.unitInfo.childUnits)
    ...
    updateUnit () {
      if (this.unitInfo.name.trim() === '' || this.unitInfo.name === undefined) {
        this.$Message.error('请填写机构名称')
        return
      }
      ...
      let that = this
      this.$http.post('/api/pc/tree/updateUnit',
        {id: this.unitInfo.id, name: this.unitInfo.name}, this.options)
        .then(function (response) {
          that.$Message.success(response.body.msg)
          if (response.body.ret === 0) {
    ...
    remove (root, node, data) {
      ...
      this.$Modal.confirm({
        ...
        onOk () {
          that.$http
            .post('/api/pc/tree/deleteUnit', {id: node.node.id}, that.options)
            .then(function (response) {
              that.$Message.success(response.body.msg)
              if (response.body.ret === 0) {
